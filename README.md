...
- 음성 명령으로 현재 상품 페이지의 장바구니 버튼 자동 클릭
- 음성 명령으로 알라딘 장바구니 페이지 이동
...
- “장바구니에 넣어줘”, “바구니에 넣어줘”, “카트에 넣어줘” 등 장바구니 담기 음성 명령 파싱 구현
- 알라딘 상품 페이지에서 JavaScript 기반 장바구니 버튼 탐색 및 클릭 구현
- 상품 페이지가 아니거나 장바구니 버튼이 없을 때 TTS 오류 안내 구현
- “장바구니 보여줘”, “장바구니로 가줘”, “장바구니로 이동” 등 장바구니 이동 음성 명령 구현
- 상단 WebView에서 알라딘 장바구니 페이지 열기 구현
- 결제 자동화는 구현하지 않음
...
4. 마이크 권한을 허용합니다.
5. 하단 전체 영역의 큰 음성 버튼을 누르고 “채식주의자 찾아줘”처럼 말합니다.
6. 검색 결과가 나오면 “1번 선택”이라고 말하거나 결과 항목을 터치합니다.
7. 상품 페이지가 열린 뒤 “장바구니에 넣어줘”라고 말하면 WebView 안의 장바구니 버튼 클릭을 시도합니다.
8. “장바구니 보여줘”라고 말하면 알라딘 장바구니 페이지로 이동합니다.
...
8. 하단 전체 영역의 큰 `말하기` 버튼을 누르고 “채식주의자 찾아줘”처럼 말한 뒤 손을 뗍니다.
...
11. 하단 전체 영역의 큰 `말하기` 버튼을 누르고 검색어를 말합니다.
...
1. 앱을 실행합니다.
2. 마이크 권한을 허용합니다.
3. 하단 전체 영역의 큰 `말하기` 버튼을 누른 채 “1984 찾아줘” 또는 “채식주의자 찾아줘”라고 말합니다.
...
현재 테스트는 규칙 기반 발화 파싱, 번호/제목 선택, 장바구니 담기/이동 명령 파싱, `1984 찾아줘` 검색어 처리, API 키 없음 상태 처리, `mallType == EBOOK` 필터링, 검색 결과 안내 문구, ViewModel 상태 전환을 포함합니다.

## 접근성 고려사항

- 주요 버튼과 검색 결과에 `contentDescription`을 설정했습니다.
- 음성 검색 버튼은 우측 하단의 작은 FAB가 아니라 화면 하단 전체 영역의 대형 버튼입니다.
...
이 버전은 선택한 전자책의 알라딘 상품 페이지 열기, 장바구니 담기, 장바구니 페이지 이동까지만 구현합니다. 결제 실행, 결제하기 버튼 자동 클릭, 삼성페이 자동 선택, 좌표 기반 클릭, 화면 이미지 인식 방식은 구현하지 않았습니다. 결제 단계는 민감도가 높고 사용자 확인이 필수이므로 MVP 명령 모델에 포함하지 않았습니다.
